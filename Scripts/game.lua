...
local function createBullet( bullet, x, y, w, h )
  ...
  thisBullet:applyForce( 0, -0.666, x, y)
...
local function loopBullet()
  if ship.level == 1 then
    bullet = createBullet( bullet, ship.x, ship.y - 30, 5, 20 )
  elseif ship.level == 2 then
    bullet2 = createBullet( bullet2, ship.x + 15, ship.y, 5, 20 )
    bullet3 = createBullet( bullet3, ship.x - 15, ship.y, 5, 20 )
  elseif ship.level  == 3 then
    bullet = createBullet( bullet, ship.x, ship.y - 30, 5, 20 )
    bullet2 = createBullet( bullet2, ship.x + 30, ship.y, 5, 20 )
    bullet3 = createBullet( bullet3, ship.x - 30, ship.y, 5, 20 )
  elseif ship.level == 100 then
  else
    bullet = createBullet( bullet, ship.x, ship.y - 30, 5, 20 )
  end
...
function createEmitter( filePath, x, y )
...
function scene:show( event )
  ...
  if ( phase == "will" ) then
    ...
    local emitter = createEmitter(filePath, centerX, centerY)
  ...
  elseif ( phase == "did" ) then
    ...
    bulletLoop = timer.performWithDelay( shotSpeed, loopBullet, 0 )
