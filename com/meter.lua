...
function public.create_fps()
	...
	fpsMeter.back = display.newRect( fpsMeter, left + 2, top + 2, 50, 15 )
...
function public.create_mem()
	...
	local hudFrame = display.newRect( hud, 0, 0, 100, 20)
	hudFrame:setFillColor(0.2,0.2,0.2)
	hudFrame:setStrokeColor(1,1,0)
	hudFrame.strokeWidth = 1
  hudFrame.alpha = 0
	hudFrame.x = right - hudFrame.contentWidth/2
	hudFrame.y = top + hudFrame.contentHeight/2

	local mMemLabel = display.newText( hud, "Main Mem:", hudFrame.x - hudFrame.contentWidth/2 + 10, hudFrame.y + 10, native.systemFont, 12 )
	mMemLabel:setFillColor(1,0.4,0)
	mMemLabel.anchorX = -1

	local tMemLabel = display.newText( hud, "Texture Mem:", hudFrame.x - hudFrame.contentWidth/2 + 10, hudFrame.y, native.systemFont, 12 )
